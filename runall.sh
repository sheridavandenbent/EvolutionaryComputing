                        ...
                        do
                            ...
                            do
                                ...
                                do
                                    ...
                                    do
                                        echo "\n"
                                        java -DPARENTSELECT_STYLE="$PARENTSELECT_STYLE" -DREPRODUCE_STYLE="$REPRODUCE_STYLE" -DSURVIVAL_STYLE="$SURVIVAL_STYLE" -Dq="$q" -Dpop_size="$pop_size" -Doff_size="$off_size" -Dstd_dev_th="$std_dev_th" -Dtournament_size="$tournament_size" -Dprob_pick_best="$prob_pick_best" -jar testrun.jar -submission=player15 -evaluation=SchaffersEvaluation -seed=1

                                        for i in `seq 4`
                                        do
                                            if (( (count % proccount) == 0)); then
                                                score=`java -DPARENTSELECT_STYLE="$PARENTSELECT_STYLE" -DREPRODUCE_STYLE="$REPRODUCE_STYLE" -DSURVIVAL_STYLE="$SURVIVAL_STYLE" -Dq="$q" -Dpop_size="$pop_size" -Dstd_dev_th="$std_dev_th" -Dtournament_size="$tournament_size" -Dprob_pick_best="$prob_pick_best" -jar testrun.jar -submission=player15 -evaluation=BentCigarFunction -seed=1 | grep Score | cut -d' ' -f 2`
                                                echo "$ef,$PARENTSELECT_STYLE,$REPRODUCE_STYLE,$SURVIVAL_STYLE,$pop_size,$std_dev_th,$tournament_size,$prob_pick_best,$score" >> $ef-statistics.txt
                                            fi
                                            count=$((count+1))
                                        done
                                    done
